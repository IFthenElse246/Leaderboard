        ...
        for name in queue.iter() {
            ...
            if let Some(board) = lock.get(name) {
                ...
                let handle = match File::create(&temp_path) {
                    Ok(v) => v,
                    Err(err) => {
                        let _ = writeln!(&mut io::stderr().lock(), "Failed to open temp file to save leaderboard backup.\n{}",err);
                        break;
                    }
                };
                ...
                match ciborium::into_writer(&new_tree, handle) {
                    Ok(_) => {
                        if let Err(err) = std::fs::rename(&temp_path, saves_path.join(format!("{name}.cbor"))) {
                            let _ = writeln!(&mut io::stderr().lock(), "Failed to rename temp file into save.\n{}",err);
                            break;
                        }
                    },
                    Err(err) => {
                        let _ = writeln!(&mut io::stderr().lock(), "Failed to write to temp file to save leaderboard backup.\n{}",err);
                        break;
                    }
                }
            }
...
pub fn execute_action(action: ActionType, interaction: Interaction, dat: String) -> Result<String, Status> {
    match action {
